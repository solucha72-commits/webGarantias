...
export default function Informes() {
  ...
  const enviarPorEmail = async () => {
    ...
    setEnviando(true);
    try {
      const RESEND_API_KEY = "re_Qnh4thnV_HraBizJRrMFy3f9hN3ju3w61";
              ...
              <div class="content">
                ...
                <div class="filtros">
                  ...
                  <div class="filtro-item">
                    <span class="label">Familia:</span> ${filtroTipo || "Todas"}
                  </div>
                  <div class="filtro-item">
                    <span class="label">Marca:</span> ${filtroMarca || "Todas"}
                  </div>
                  <div class="filtro-item">
                    <span class="label">Centro:</span> ${filtroCentro || "Todos"}
                  </div>
                </div>
      ...
      if (response.ok && (data.id || data.success)) {
        alert(`✅ Informe enviado correctamente a ${email}`);
        setModalEmail(false);
        setEmail("");
      } else {
        alert(`❌ Error: ${data.message || "No se pudo enviar el email"}`);
      }
    } catch (error: any) {
